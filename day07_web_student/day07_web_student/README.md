...
- 已完成功能：所有TODO
- 选择的拓展任务：B（生命周期详情页）
- 拓展访问或运行方法：`python app.py` 启动后登录（student / day07），访问 `http://127.0.0.1:5000/segments`，或点击顶部导航“生命周期”进入；页面右上角“返回数据看板”可回到看板。
- 关键实现位置：`app.py` 中的 `segments()` 路由；`services/data_service.py` 中的 `load_segment_data()`；`templates/segments.html`；`templates/base.html` 中的导航链接；`static/css/style.css` 中的条形图样式。
- 拓展证据文件：`screenshots/05_extension.png`
- 尚未解决的问题：无。拓展各项验收点（路由、继承 base.html、阶段/用户数/流失人数/流失率展示、带数值证据的数据观察、导航往返）均已在本地运行验证通过。
