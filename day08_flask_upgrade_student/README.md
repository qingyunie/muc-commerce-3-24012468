...
## 学生信息
 ...
 姓名：王一维
- 学号：24012468
- 已完成路由或接口：`/health`（无需登录）、`/api/metrics`（登录后返回四张指标卡，数值来自数据服务读取CSV计算）、`/api/categories`（支持`?category=`筛选，无效品类返回400统一错误JSON）、统一400错误处理（`{"ok": false, "error": "请求格式不正确。"}`）
- 测试文件：`tests/test_api.py`（共5条：`/health`返回200、未登录访问`/api/metrics`被重定向拦截、登录后`/api/metrics`返回ok与metrics、`/api/categories?category=Fashion`返回1行筛选结果、非法品类返回400错误JSON）
- 尚未解决的问题：无