...
@login_required
def metrics_api():
    # 指标卡数据全部由数据服务读取CSV计算，接口层不写死任何数值，
    # 每条记录保留label、value、note三个字段。
...
@login_required
def categories_api():
    # category查询参数真正传入数据服务的筛选逻辑，
    # "全部"返回所有品类，具体品类只返回对应行。
    category = request.args.get("category", "全部").strip() or "全部"
    rows = load_category_api_data(BASE_DIR, category)
    if category != "全部" and not rows:
        # 筛选条件不存在时走统一的400错误结构，不用200伪装失败。
        abort(400)
...
def bad_request(_error):
    # 统一JSON错误结构，状态码保持400，不用200伪装失败请求。
